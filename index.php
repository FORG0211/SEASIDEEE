...
// --- DISPLAY RESERVATIONS WITH ORDERS ---
...
if (isset($_SESSION['user_id'])) {
    ...
    try {
        $stmt_user_details = $pdo->prepare("SELECT name, phone_number FROM users WHERE id = ?");
        $stmt_user_details->execute([$current_user_id]);
        $user_data = $stmt_user_details->fetch();

        if ($user_data) {
            $current_user_name_display = htmlspecialchars($user_data['name'], ENT_QUOTES);
            ...
            $stmt_reservations->execute([
                ':name' => $user_data['name'],
                ':phone' => (string)$user_data['phone_number']
            ]);
            ...
            foreach ($reservations as $res) {
                ...
                foreach ($orders_for_this_reservation as $item) {
                    $total_order_price_for_reservation += $item['item_price'] * $item['quantity'];
                }

            }
        } else {
            $page_error_message = "Unable to retrieve your account info.";
        }
    } catch (PDOException $e) {
        error_log("Error retrieving user reservations: " . $e->getMessage());
        $page_error_message = "An error occurred while fetching your reservations.";
    }
}
...
<head>
    ...
    font-weight: 500;
    color: white;
    background-color: #17a2b8; /* Teal-like blue */
    border: none;
    border-radius: var(--border-radius-sm);
    cursor: pointer;
    margin-right: 0.5rem;
